...
public class FragmentDeniedList extends BaseFragment implements BlockedListAdapter.OnViewClickListener {
  ...
  private List<Profile> mDeniedProfiles = new ArrayList<>();
  ...
  public void onResume() {
    ...
    ((AppCompatActivity) getActivity()).getSupportActionBar().setTitle(R.string.deny_contact_title);
  ...
  private void showDeniedUsers(List<Profile> pageList) {
    loading.setVisibility(View.GONE);
    mDeniedProfiles.clear();

    notifyProfilesAdapter();

    mDeniedProfiles.addAll((List<Profile>) pageList);

    mAdapter = new BlockedListAdapter(getContext(), mDeniedProfiles, BlockedListAdapter.MODE_ALLOW);
  ...
  public void loadFirstPage() {
    ...
    mCurrentPage = 1;
    Pagination.getDeniedList(mCurrentPage, getContext(), new IPaginationCallback() {
      @Override
      public <T> void onPageReceived(List<T> pageList) {
        ...
        showDeniedUsers((List<Profile>) pageList);
  ...
  private void loadMorePages() {
    Pagination.getDeniedList(mCurrentPage, getContext(), new IPaginationCallback() {
      ...
      public <T> void onPageReceived(List<T> pageList) {
        ...
        {
          mAdapter.insertInternal(mDeniedProfiles, post, mDeniedProfiles.size());
        }
    ...
    switch (viewId)
    {
      ...
      case R.id.button_following:
        ...
        builder.positiveActionClickListener(new View.OnClickListener() {
          @Override
          public void onClick(View v) {
            HttpClient.get(String.format(Constants.Server.Profile.GET_ALLOW, profile.getId()), new AsyncHttpResponser(getContext()) {
              @Override
              public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                super.onSuccess(statusCode, headers, responseBody);
                builder.dismiss();
                removeFromAdapterDataSource(profile.getId());
                mAdapter.mProfiles = mDeniedProfiles;
  ...
  private void removeFromAdapterDataSource(String memberId) {
    for (Profile deniedProfile : mDeniedProfiles)
    {
      if (deniedProfile.getId().equals(memberId))
      {
        mDeniedProfiles.remove(deniedProfile);
        break;
      }
    }
