...
import com.yalantis.ucrop.util.CompatUtils;
...
public class SmoothCheckBox extends View implements Checkable {
    ...
    private static final int COLOR_UNCHECKED = Color.argb(160, 0, 0, 0);
    private static final int COLOR_CHECKED   = Color.parseColor("#FB4846");
    private static final int COLOR_FLOOR_UNCHECKED = Color.argb(100,222,222,222);
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.ucrop_SmoothCheckBox);
        int tickColor = ta.getColor(R.styleable.ucrop_SmoothCheckBox_ucrop_color_tick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.ucrop_SmoothCheckBox_ucrop_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.ucrop_SmoothCheckBox_ucrop_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(R.styleable.ucrop_SmoothCheckBox_ucrop_color_checked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(R.styleable.ucrop_SmoothCheckBox_ucrop_color_unchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.ucrop_SmoothCheckBox_ucrop_stroke_width, CompatUtils.dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setChecked(boolean checked, int noListener) {
        mChecked = checked;
        reset();
        invalidate();
        if (noListener == 0) {
     ...
     * @param animate change with animation
     */
    public void setChecked(boolean checked, boolean animate, int noListener) {
        if (animate) {
            mTickDrawing = false;
            mChecked = checked;
            mDrewDistance = 0f;
            ...
            this.setChecked(checked, noListener);
        }
    }
    ...
    private void startUnCheckedAnimation() {
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                mScaleVal = (float) animation.getAnimatedValue();
                mFloorColor = Color.argb(100,222,222,222);
                postInvalidate();
            }
        });
        animator.start();
